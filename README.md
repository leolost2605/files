...
To install, use `ninja install`, then execute with `io.github.leolost2605.files`
...
```bash
ninja io.github.leolost2605.files-pot
ninja io.github.leolost2605.files-update-po
```
...
```bash
ninja -C build io.github.leolost2605.files-update-po
```
...
```bash
flatpak-builder build io.github.leolost2605.files.yml --user --install --force-clean --install-deps-from=appcenter
```

Then execute with

```bash
flatpak run io.github.leolost2605.files
```

### Uninstall

```bash
flatpak uninstall io.github.leolost2605.files --user
```